...
impl MultiplicativeFriFolder {
    pub fn compute_next_fri_layer<F: Field>(
    ...
    pub fn compute_next_fri_layer_in_place<F: Field>(
        domain: &dyn FftDomainBase,
        values: &[F],
        eval_point: &F,
        output_layer: &mut [F],
    ...
    pub fn next_layer_element_from_two_previous_layer_elements<F: Field>(
        f_x: &F,
        f_minus_x: &F,
        eval_point: &F,
        x_inv: &F,
    ...
    fn compute_next_fri_layer_impl<F: Field>(
        domain: &FftDomain<F>,
        input_layer: &[F],
        eval_point: &F,
        output_layer: &mut [F],
