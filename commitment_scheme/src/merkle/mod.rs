...
impl<F: PrimeField, H: Hasher<F, Output = GenericArray<u8, U32>>> MerkleTree<F, H> {
    ...
    pub fn verify_decommitment<P: Prng, W: Digest>(
        ...
        let (mut node_idx, mut node_hash) = queue.front()?;
        ...
        while node_idx != 1 {
            ...
            if !queue.is_empty() && queue.front()?.0 == sib_node_idx {
                sib_node_hash = queue.front()?.1;
                queue.pop_front();
            } else {
                let decommitment_node = channel.recv_decommit_node();
...
mod tests {
    ...
    where
        F: PrimeField,
        H: Hasher<F, Output = GenericArray<u8, U32>>,
    {
        ...
        H: Hasher<F, Output = GenericArray<u8, U32>>,
    {
        ...
        while queries.len() < num_queries {
            ...
            queries.push(query);
            query_data.push((query, data[query]));
        }
